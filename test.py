...
def show_executive_dashboard(data, cluster_summary):
    ...
    with col1:
        st.markdown(create_alert_box(
            f"There are {len(data[data['risk_category'] == 'IMMEDIATE'])} members at immediate risk of churning in the next 30 days.",
            "danger"
        ), unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown(create_alert_box(
            f"Average customer lifetime value is ${avg_ltv:,.2f}, up 15% from last quarter.",
            "success"
        ), unsafe_allow_html=True)
